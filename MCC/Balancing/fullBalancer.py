...
class FullBalancer(Balancer):
    ...
    Args:
        model (core.ModelInterface): ModelInterface of the model which we intend to balance.
        ...
        target_model (core.ModelInterface): ModelInterface of the model which will be used as a target. The algorithm will try to assign as many 
    ...
    def _generate_metabolite_assertion(self, metabolite):
        ...
        Args:
            metabolite (core.Metabolite): metabolite for which to generate the assertion.
        """
        element_symbols = {}
        constraints = []
    ...
    def assign_from_solver(self, sat_model = None):
        """
        Translate balancing assignments from the z3 variables.

        Args:
            sat_model(z3.model): Model from which to take the assignments.
        """
        m = self.solver.model() if sat_model is None else sat_model
        for metabolite in self.model_interface.metabolites.values():
            for element in self.relevant_elements:
        ...
        # add all reactions which were unbalancable
        for reaction in self.model_interface.reactions.values():
            if not reaction.is_balanced():
                if not reaction in self.unbalancable_reactions:
                    self.reaction_reasons[reaction.id] = [reaction.id]
    ...
    def _calculate_cH_equivalents(self, reduce = True):
        ...
        Returns:
            If reduce: Dictionary mapping metabolite ids to lists of assignments which all differ in more than their protonation
                =>{metabolite_id: set((formula : core.Formula, charge : int))}
            ...
            Otherwise: Dictionary mapping metabolite ids to dictionaries which then map the different non-hydrogen formulae to dictionaries,
                ...
                =>{metabolite_id: {nonH_formula  :{(hydrogen - charge) : set((formula : core.Formula, charge : int))}}}
        """
